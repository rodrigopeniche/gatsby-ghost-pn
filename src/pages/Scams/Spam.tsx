...
const IndexPage = ({ data }: { data: any }) => {
    ...
    return (
        <>  {!subdomain ? null :
                <SEO
                    title={`What is Phone Spam? | California.Phonenumbers.org`}
                    keywords={`Phone spam, spam calls, spam messaging, sms spam, spam phone calls, legal phone spam, voip spam, internet call spam, voip call spam, spam over internet technology, spit, avoid sms spam, avoid spam messages, avoid spam calls, block phone numbers, at&t call protect, verizon call filter, t-mobile scam shield, sprint call screener, google call blocker, ios call blocking`}
                    description={`Phone spam is a common method for less scrupulous telemarketers across the country. There are several ways to combat this method of advertisement, and knowing how can significantly decrease the number of spam calls you receive. Learn more here.`}
                    path={'/Scams/Spam'}
                    ArticleData={{title: "What is Phone Spam?"}}
                />
            }
            <BreakpointProvider queries={queries}>
                <Frame>
                    <MainPageContainer>
                        ...
                        <SearchBox />
                        { !subdomain ? null :
                            <Article data={articles[subdomain]["scams-spam"]} />
                        }
                    </MainPageContainer>
                </Frame>
                <Footer />
