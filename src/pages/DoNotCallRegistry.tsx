...
const IndexPage = ({ data }: { data: any }) => {
    ...
    return (
        <>  {!subdomain ? null :
                <SEO
                    title={`What is the California Do Not Call Registry? | California.Phonenumbers.org`}
                    keywords={`California do not call registry, california unwanted calls, california DNC registry, california robocalls, stop california spam calls, stop california robocalls, register on california DNC, register on california do not call registry, california national do not call registry, california telemarketing, california telemarketers`}
                    description={`California’s government offers a method for reducing solicitor calls which is called the do not call registry. Citizens of the Golden State may register on the list to officially request they not be called by unknown numbers. Learn how to register here.`}
                    path={'/DoNotCallRegistry'}
                    ArticleData={{title: "What is the California Do Not Call Registry?"}}
                />
            }
            <BreakpointProvider queries={queries}>
                <Frame>
                    <MainPageContainer>
                        ...
                        <SearchBox />
                        { !subdomain ? null :
                            <Article data={articles[subdomain]["do-not-call-registry"]} />
                        }
                    </MainPageContainer>
                </Frame>
                <Footer />
